...
const { potch_instances } = require('../blueprints')
const article = {}
...
article.FIND_BY_ID = async (req, res, next) => {
    try {
        const result = await potch_instances.viewArticle(req.params.id)
...
article.COUNT_ARTICLE = async (req, res, next) => {
    try {
        let q = req.query
        const result = await potch_instances.countDocuments({
...
article.COUNT_IF_EXIST = async (req, res, next) => {
    try {
        const result = await potch_instances.countDocuments(req.body)
...
article.STORE = async (req, res, next) => {
    try {
        const result = await potch_instances.storeArticle(req.body)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.UPDATE = async (req, res, next) => {
    try {
        const result = await potch_instances.updateArticle(req.body, req.params.id)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.DELETE = async (req, res, next) => {
    try {
        const result = await potch_instances.deleteArticle(req.params.id)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.LIST = async (req, res, next) => {
    try {
        const result = await potch_instances.listArticle(req)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.DATATABLES = async (req, res, next) => {
    try {
        ...
        let totalDoc = await potch_instances.countDocuments()

        potch_instances.dataTables({
...
article.CUSTOM_QUERY = async(req, res, next) => {
    try {
        ...
        const result = await potch_instances.find(filter, fields).populate(website_query).limit(parseInt(limit)).skip(parseInt(offset)).sort(sorting)
        res.status(200).send({'data': result})
    } catch (error) {
        console.log(error)
        next(createError(error))
