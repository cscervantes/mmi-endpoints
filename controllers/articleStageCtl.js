...
const { article_stages } = require('../blueprints')
const article = {}
...
article.FIND_BY_ID = async (req, res, next) => {
    try {
        const result = await article_stages.viewArticle(req.params.id)
...
article.COUNT_ARTICLE = async (req, res, next) => {
    try {
        let q = req.query
        const result = await article_stages.countDocuments({
...
article.COUNT_IF_EXIST = async (req, res, next) => {
    try {
        const result = await article_stages.countDocuments(req.body)
...
article.STORE = async (req, res, next) => {
    try {
        const result = await article_stages.storeArticle(req.body)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.UPDATE = async (req, res, next) => {
    try {
        const result = await article_stages.updateArticle(req.body, req.params.id)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.DELETE = async (req, res, next) => {
    try {
        const result = await article_stages.deleteArticle(req.params.id)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.LIST = async (req, res, next) => {
    try {
        const result = await article_stages.listArticle(req)
        res.status(200).send({'data': result})
    } catch (error) {
        next(createError(error))
    }
...
article.DATATABLES = async (req, res, next) => {
    try {
        ...
        let totalDoc = await article_stages.countDocuments()

        article_stages.dataTables({
...
article.CUSTOM_QUERY = async(req, res, next) => {
    try {
        ...
        const result = await article_stages.find(filter, fields).populate(website_query).limit(parseInt(limit)).skip(parseInt(offset)).sort(sorting)
        res.status(200).send({'data': result})
    } catch (error) {
        console.log(error)
        next(createError(error))
